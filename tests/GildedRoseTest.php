...
class GildedRoseTest extends TestCase
{
    ...
    {
        $items = [new Item('', 0, GildedRose::MIN_QUALITY)];

        $gilded_rose = new GildedRose($items);
        $gilded_rose->update_quality();

        $this->assertEquals(GildedRose::MIN_QUALITY, $items[0]->quality);
    }
    ...
    {
        $items = [new Item(GildedRose::AGED_BRIE, 0, 5)];
    ...
    {
        $items = [new Item(GildedRose::AGED_BRIE, 0, GildedRose::MAX_QUALITY)];

        $gilded_rose = new GildedRose($items);
        $gilded_rose->update_quality();

        $this->assertEquals(GildedRose::MAX_QUALITY, $items[0]->quality);
    }
    ...
    {
        $items = [new Item(GildedRose::SULFURAS, 10, 10)];
    ...
    {
        $items = [new Item(GildedRose::BACKSTAGE, $sellIn, $quality)];
